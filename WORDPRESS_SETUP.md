...
### 6b. Comments — enable anonymous commenting (one small code snippet needed)
...
**This alone is NOT enough — one more step is required.** Since WordPress
4.7, the REST API blocks anonymous (guest) comment creation by default,
completely independently of the Discussion Settings checkbox above — that
checkbox only affects the legacy comment form (`wp-comments-post.php`), not
the REST endpoint this site actually uses. There's no UI toggle for this;
it's a filter you add via a code snippet:
...
Add this either:
- As a one-liner in your active theme's `functions.php`, **or**
- As a must-use plugin (more durable — survives theme changes/updates):
  drop a `.php` file containing the line above into
  `wp-content/mu-plugins/` (create that folder if it doesn't exist; no
  activation step needed for mu-plugins)

Without this filter, every guest comment submission fails with exactly:
`{"code":"rest_comment_login_required","message":"Sorry, you must be
logged in to comment.","status":401}` — even with the Discussion Settings
above correctly configured. Reference:
...
the visitor when "Comment must be manually approved" is on, and appear on
the site once you approve them from WordPress → Comments.
