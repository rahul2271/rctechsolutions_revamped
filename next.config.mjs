...
const nextConfig = {
  ...
  async redirects() {
    return [
      // STILL EMPTY — this is worth fixing before the next migration, not
      // just noting. If any blog/project slugs changed during the
      // Firestore → WordPress cutover, Google is currently treating those
      // old URLs as 404s (or new URLs as having zero history) instead of
      // carrying forward whatever ranking/crawl trust they'd built up —
      // this alone can suppress web (not image) ranking for months.
      //
      // How to find out if this affects you (5 minutes):
      // 1. Search Console → Indexing → Pages → filter "Not indexed" →
      //    look for old-pattern URLs no longer live on the site.
      // 2. Search Console → Indexing → Pages → "Page with redirect" /
      //    "404" reasons — any of YOUR OWN old blog URLs showing up here
      //    need an entry below.
      // 3. For each one: { source: "/old-slug", destination: "/new-slug", permanent: true },
      //
      // Example shape once you have real mappings:
      // { source: "/blog/old-post-title", destination: "/blogs/new-post-title", permanent: true },
    ];
  },
