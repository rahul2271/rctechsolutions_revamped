            ...
            <article className="min-w-0">
              ...
              {blogImage && (
                <div className="relative w-full mb-10 overflow-hidden rc-blueprint-card" style={{ aspectRatio: "16/9" }}>
                  <Image src={blogImage} alt={blog.title} fill className="object-cover" priority sizes="(max-width: 1024px) 100vw, 70vw" />
                </div>
              )}
                      ...
                      <Link key={rp.id} href={`/blogs/${rp.slug}`} className="group flex flex-col bg-[var(--rc-paper)] hover:bg-white transition-colors">
                        {getImageSource(rp) && (
                          <div className="relative w-full overflow-hidden" style={{ aspectRatio: "16/9" }}>
                            <Image src={getImageSource(rp)} alt={rp.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="33vw" />
                          </div>
                        )}
                        <div className="p-4">
                          ...
                          <p className="rc-mono text-[0.6rem] mt-1.5 text-[rgba(42,45,53,0.5)]">{readingTime(rp.content || "")}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
