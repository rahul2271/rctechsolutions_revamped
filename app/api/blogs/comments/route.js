...
export async function POST(req) {
  try {
    ...
    if (!res.ok) {
      // TEMP DIAGNOSTIC — remove once the "must be logged in" issue is
      // resolved. This prints exactly what WordPress sent back (including
      // its internal error `code`, not just the human-readable `message`)
      // to your terminal running `next dev`/`next start`. If you're still
      // seeing "Sorry, you must be logged in to comment." after turning
      // off Discussion Settings → "Users must be registered and logged in
      // to comment", check this log for:
      //   1. data.code === "rest_comment_login_required" confirms it's
      //      genuinely WordPress's core comment_registration check firing
      //      — meaning the setting change hasn't taken effect server-side
      //      yet (a caching plugin or host-level cache on Hostinger is the
      //      most common cause — clear any WP cache plugin, or check if
      //      one is installed under Plugins).
      //   2. A DIFFERENT data.code (e.g. from a security plugin like
      //      Wordfence, or a "Disable REST API" plugin) means something
      //      other than core Discussion Settings is blocking this —
      //      check Plugins for anything REST-API or security related.
      //   3. Compare the WP_URL logged below against the exact WordPress
      //      site where you changed the setting — if you have more than
      //      one *.hostingersite.com install, it's easy to edit settings
      //      on the wrong one.
      console.error("[comments POST] WordPress rejected the comment:", {
        wpUrl: `${WP_URL}/wp-json/wp/v2/comments`,
        status: res.status,
        code: data.code,
        message: data.message,
        wpAuthConfigured: Boolean(WP_AUTH),
      });
